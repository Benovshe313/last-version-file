...
public:
    ...
    void saveDataAdmin() {
        ofstream file("admin_data.txt", ios::trunc);
        if (file.is_open()) {
            file << mobNum << " " << passWord << " " << budget << " " << moneyEarn << " " << moneySpend << endl;
            file.close();
        }
    }
    ...
    void loadDataAdmin() {
        ...
        if (file.is_open()) {
            file >> mobNum >> passWord >> budget >> moneyEarn >> moneySpend;
            file.close();
        }
    }
...
public:
    ...
    void saveDataUser() {
        ofstream file("user_data.txt", ios::trunc);
        if (file.is_open()) {
            file << userMobNum << " " << userPassword << endl;
            file.close();
        }
    }
    void loadDataUser() {
            ...
            for (int j = 0; j < admin.foodCount; j++) {
                if (user.foodNames[i] == admin.foods[j].name) {
                    ...
                    for (int k = 0; k < admin.foods[j].ingredientCount; k++) {
                        ...
                        admin.moneySpend += (admin.foods[j].ingredients[k].count * admin.foods[j].ingredients[k].price) + 10;
                        for (int m = 0; m < admin.ingredientCount; m++) {
                            if (admin.foods[j].ingredients[k].name == admin.ingredients[m].name) {
